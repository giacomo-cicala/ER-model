...
Analizziamo il caso della rimozione casuale di una frazione $f$ di nodi. Utilizzando il criterio di Molloy-Reed \cref{sec:molloy_reed}:
\begin{equation}
    \frac{\langle k^2 \rangle}{\langle k \rangle} > 2
\end{equation}
il quale determina la condizione necessaria affinché esista una componente gigante per un generico network con distribuzione dei gradi $P(k)$, è possibile derivare la soglia critica di percolazione $f_c$ \cref{sec:perc_tresh}:
...
\begin{equation*}
    \tilde{\chi_\alpha}^2 = 55 \quad\quad \alpha_{\text{fit}} = 0.985 \pm 0.001 \quad\quad \alpha_{\text{teo}} = 1
\end{equation*}
...
\subsection{Criterio di Molloy-Reed}\label{sec:molloy_reed}
L'obiettivo di questa sezione è derivare il criterio di Molloy-Reed, il quale permette di calcolare la soglia di percolazione per una rete arbitraria \cite{barabasi2016network}. Affinché esista una componente gigante, ogni nodo che vi appartiene deve essere connesso, in media, ad almeno altri due nodi. Di conseguenza, il grado medio $k_i$ di un nodo $i$ scelto casualmente all'interno della componente gigante deve essere almeno pari a 2.

Indichiamo con $P(k_i \mid i \leftrightarrow j)$ la probabilità condizionata che un nodo della rete abbia grado $k_i$, sapendo che esso è connesso a un nodo $j$ appartenente alla componente gigante. Questa probabilità condizionata ci permette di determinare il grado atteso del nodo $i$ come:
\begin{equation}
    \langle k_i \mid i \leftrightarrow j \rangle = \sum_{k_i} k_i P(k_i \mid i \leftrightarrow j) = 2
    \label{eq:cond_degree}
\end{equation}
In altre parole, $\langle k_i \mid i \leftrightarrow j \rangle$ deve eguagliare o superare il valore 2, che è la condizione affinché il nodo $i$ faccia parte della componente gigante in espansione (un link per essere raggiunto, almeno un link per continuare la catena). 

Possiamo riscrivere la probabilità che compare nella sommatoria \eqref{eq:cond_degree} applicando il Teorema di Bayes:
\begin{equation}
    P(k_i \mid i \leftrightarrow j) = \frac{P(i \leftrightarrow j \mid k_i) P(k_i)}{P(i \leftrightarrow j)}
\end{equation}
Per una rete con distribuzione dei gradi $P(k)$, in assenza di correlazioni di grado, possiamo esprimere le probabilità di connessione come:
\begin{equation}
    P(i \leftrightarrow j) = \frac{2L}{N(N-1)} = \frac{\langle k \rangle}{N-1}, \quad\quad P(i \leftrightarrow j \mid k_i) = \frac{k_i}{N-1}
\end{equation}
Queste espressioni riflettono il fatto che possiamo scegliere tra $N-1$ nodi a cui collegarci, ciascuno con probabilità $1/(N-1)$, e che possiamo tentare questa operazione $k_i$ volte. Sostituendo questi termini nell'equazione del valore atteso \eqref{eq:cond_degree}, otteniamo:
\begin{align}
    \sum_{k_i} k_i P(k_i \mid i \leftrightarrow j) &= \sum_{k_i} k_i \frac{P(i \leftrightarrow j \mid k_i) P(k_i)}{P(i \leftrightarrow j)} \nonumber \\
    &= \sum_{k_i} k_i \frac{k_i P(k_i)}{\langle k \rangle} \nonumber \\
    &= \frac{1}{\langle k \rangle} \sum_{k_i} k_i^2 P(k_i) = \frac{\langle k^2 \rangle}{\langle k \rangle}
\end{align}
Imponendo la condizione iniziale per cui tale valore atteso deve essere strettamente maggiore di 2 per sostenere la componente gigante, arriviamo al criterio di Molloy-Reed:
\begin{equation}
    \frac{\langle k^2 \rangle}{\langle k \rangle} > 2
\end{equation}
\subsection{Soglia critica di percolazione}\label{sec:perc_tresh}
Studiamo ora la robustezza della rete a fronte della rimozione casuale di una frazione $f$ di nodi \cite{barabasi2016network}. I nodi rimanenti costituiscono una frazione $p = 1 - f$ della rete originaria. 

Per comprendere come si modificano i momenti della distribuzione dei gradi, consideriamo innanzitutto un singolo nodo che nella rete intatta aveva esattamente grado $k$. 
Ogni suo vicino ha una probabilità $p = 1-f$ di non essere rimosso. Dunque la probabilità che un nodo di grado $k$ abbia grado $k'$ dopo la rimozione casuale è descritta da una distribuzione binomiale con $k$ tentativi e probabilità di successo $1-f$:
\begin{equation}
    P(k' \mid k) = \binom{k}{k'} f^{k - k'} (1-f)^{k'}
\end{equation}

Ricordando che per una distribuzione binomiale il valore atteso è $np$ e la varianza è $np(1-p)$, nel nostro caso il numero di tentativi corrisponde al grado originario ($n = k$) e la probabilità di successo alla sopravvivenza del vicino ($p = 1-f$). Per un nodo partente da grado $k$ si ha quindi:
\begin{itemize}
    \item $\langle k' \mid k \rangle$: $k(1-f)$
    \item $\text{Var}(k' \mid k)$: $k(1-f)f$
\end{itemize}
Ricordando in generale che $ \sigma^{2} = \langle x^2 \rangle - \langle x \rangle^2 $, possiamo calcolare il secondo momento condizionato alla rimozione casuale:
\begin{equation}
    \langle k'^2 \mid k \rangle = \text{Var}(k' \mid k) + \langle k' \mid k \rangle^2 = k(1-f)f + k^2(1-f)^2
\end{equation}

A questo punto, per ottenere le quantità macroscopiche riferite all'intera rete danneggiata, è sufficiente calcolare la media pesata di questi risultati su tutti i possibili gradi iniziali $k$, moltiplicandoli per la probabilità $P(k)$ della distribuzione originaria.

Per il nuovo grado medio $\langle k' \rangle$:
\begin{equation}
    \langle k' \rangle = \sum_{k} P(k) [k(1-f)] = (1-f) \sum_{k} k P(k)
\end{equation}
Riconoscendo che $\sum_{k} k P(k)$ è esattamente la definizione del grado medio iniziale $\langle k \rangle$, otteniamo:
\begin{equation}
    \langle k' \rangle = (1-f) \langle k \rangle
    \label{eq:k_prime}
\end{equation}
Applicando lo stesso ragionamento per il nuovo momento secondo $\langle k'^2 \rangle$:
\begin{align}
    \langle k'^2 \rangle &= \sum_{k} P(k) \left[ k(1-f)f + k^2(1-f)^2 \right] \nonumber \\
    &= f(1-f) \left[ \sum_{k} k P(k) \right] + (1-f)^2 \left[ \sum_{k} k^2 P(k) \right]
\end{align}
Anche in questo caso, riconoscendo i momenti della distribuzione originale all'interno delle parentesi quadre ($\langle k \rangle$ e $\langle k^2 \rangle$), ricaviamo l'equazione per il secondo momento della rete danneggiata:
\begin{equation}
    \langle k'^2 \rangle = f(1-f)\langle k \rangle + (1-f)^2\langle k^2 \rangle
    \label{eq:k2_prime}
\end{equation}

Per calcolare la soglia critica $f_c$ alla quale la componente gigante collassa, applichiamo il criterio di Molloy-Reed alla rete danneggiata:
\begin{equation}
    \frac{\langle k'^2 \rangle}{\langle k' \rangle} = 2
\end{equation}
Sostituendo le espressioni \eqref{eq:k_prime} e \eqref{eq:k2_prime} e isolando $f_c$, si ottiene:
\begin{equation}
    f_c = 1 - \frac{1}{\frac{\langle k^2 \rangle}{\langle k \rangle} - 1}
    \label{eq:fc_general}
\end{equation}
