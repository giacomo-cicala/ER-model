...
\begin{itemize}
    \item \textbf{Parametro d'ordine $P_\infty$:} Definito come la probabilità che un nodo appartenga alla componente gigante, coincide dunque con la frazione di nodi appartenenti alla componente gigante $S$. Questo parametro permette di distinguere una fase ordinata ($P_\infty > 0$) da una disordinata ($P_\infty = 0$). Per la teoria della percolazione si ha che \err{REF?}:
          \begin{equation}
              P_\infty = S \sim (\langle k \rangle - \langle k \rangle_c)^\beta \quad \text{per } \langle k \rangle \to \langle k \rangle_c^+
          \end{equation}
    ...
    \item \textbf{Suscettività $\chi$:} Definita come la dimensione media dei cluster piccoli $ \langle s \rangle$, rappresenta la sensibilità della rete a variazioni di $\langle k \rangle$. Per la teoria della percolazione si ha che \err{REF?}:
          \begin{equation}    \chi = \langle s \rangle \sim |\langle k \rangle - \langle k \rangle_c|^{-\gamma} \quad \text{per } \langle k \rangle \to \langle k \rangle_c
          \end{equation}
...
\begin{equation}
    P_\infty = S \approx 2(\langle k \rangle - 1)
\end{equation}
...
\begin{equation}
    P_\infty \sim (\langle k \rangle - 1)^1 \quad \text{per } \langle k \rangle \to 1^+
\end{equation}
...
\end{equation}
dove $N_s$ è il numero di cluster di dimensione $s$. Si può dimostrare che $\chi$ è dato da:
\begin{equation}
    \chi = \langle s \rangle = \frac{1}{1 - \langle k \rangle + \langle k \rangle S}
\end{equation}
Nel regime subcritico ($\langle k \rangle < 1$) si ha $S = 0$, quindi la suscettività diverge al punto critico con un un esponente critico $\gamma = 1$:
\begin{equation}
    \chi = (1- \langle k \rangle)^{-1} \quad \text{per } \langle k \rangle \to 1^-
\end{equation}
